...
#include <fcntl.h>
#include <unistd.h>
#include "qf_improved.h"
#include "murmur3.h"
#include "math.h"

#define MAX(a, b) ((a) > (b) ? (a) : (b))
#define LOW_MASK(n) ((1ULL << (n)) - 1ULL)
#define QUOTIENT_ALIGNMENT 512
...
struct quotient_filter * qf_init2(uint64_t entries, double error)
{
	struct quotient_filter * qf = malloc(sizeof(*qf));
	int q = ceil(log((double) entries * 4 / 3) / log(2));
	int r = ceil(- log(error) / log(2));
	qf_init(qf, q, r);
	return qf;
}
...
bool qf_insert2(struct quotient_filter *qf, const void * key, int key_len, uint32_t seed)
{
	uint64_t * hash = malloc(sizeof(*hash));
	MurmurHash3_x86_128(key, key_len, seed, hash);
	return qf_insert(qf, *hash);
}
...
bool qf_may_contain2(struct quotient_filter *qf, const void * key, int key_len, uint32_t seed)
{
	uint64_t * hash = malloc(sizeof(*hash));
	MurmurHash3_x86_128(key, key_len, seed, hash);
	return qf_may_contain(qf, *hash);
}
